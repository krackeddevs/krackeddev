    ...
    <div className="container mx-auto px-4 py-8 relative z-10">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {NAV_ITEMS.map((item) => (
          ...
          >
            <div
              className="relative h-full bg-black/40 border-2 border-green-400/30 p-8 transition-all duration-300 
                hover:border-green-400 hover:bg-green-400/5 backdrop-blur-sm flex flex-col items-center text-center
                hover:-translate-y-1"
              style={{
            ...
            >
              <div
                className={`mb-6 p-4 rounded-full bg-green-400/10 border-2 border-green-400/30 group-hover:scale-110 transition-transform duration-300 ${item.color}`}
              >
                <item.icon className="w-8 h-8" />
              </div>

              <h3 className="text-xl font-bold text-white mb-3 font-mono tracking-wider group-hover:text-green-400 transition-colors uppercase">
                {item.title}
              </h3>

              <p className="text-green-400/60 text-sm mb-6 leading-relaxed font-mono">
                {item.description}
              </p>
